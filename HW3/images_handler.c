...
double* read_image_from_location(char* location, char label)
{
    ...
    int limit=width * height * channels;//opt	
...
{	
	...
	for(i = 0; i < N_TRAINING_SAMPLES; i+=4)
	{
		free(training_samples[i]);
        free(training_samples[i+1]);
        free(training_samples[i+2]);
        free(training_samples[i+3]);
	}
	for(;i<N_TRAINING_SAMPLES;i++) free(training_samples[i]);
	/* Free each element of testing_samples. */
	for(i = 0; i < N_TESTING_SAMPLES; i+=4)
	{
		free(testing_samples[i]);
        free(testing_samples[i+1]);
        free(testing_samples[i+2]);
        free(testing_samples[i+3]);
    }
    for(;i<N_TESTING_SAMPLES;i++) free(testing_samples[i]);
}
